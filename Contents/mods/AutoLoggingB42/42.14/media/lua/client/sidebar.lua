...
local function setTextureWidth()
    ...
    if size == 6 then
        size = getCore():getOptionFontSizeReal()
    end
    ...
    TEXTURE_WIDTH = TEXTURE_WIDTH * 0.9
...
function ISLoggingPopup:render()
    ...
    local mouseIndex = math.floor(self:getMouseX() / TEXTURE_WIDTH)
    ...
    for i = 1, #textureNames do
        ...
        local alpha = (index == mouseIndex) and 1.0 or 0.8
        if i == 1 then alpha = 1 end

        local tex = getTexture("media/ui/Sidebar/" .. textureNames[i])

        self:drawTextureScaled(tex, index * TEXTURE_WIDTH, 0, TEXTURE_WIDTH, TEXTURE_WIDTH, alpha)
    end
end
...
function ISEquippedItem:initialise()
    ...
    if useSidebar then
        ...
        self.autoLogBtn:setImage(getTexture("media/ui/Sidebar/Autologging_Off.png"))
