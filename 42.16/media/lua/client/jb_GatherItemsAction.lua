...
function JB_GatherItemsAction:GetFirstValidDrop(destinations)
    local playerContainers = getPlayerContainers(self.character)
    if not playerContainers or not destinations then return nil, nil end

    for _, playerContainer in ipairs(playerContainers) do
        for itemType in pairs(self.itemTypes) do
            local items = playerContainer:getItemsFromFullType(itemType)

            if items and not items:isEmpty() then
                for i = 0, items:size() - 1 do
                    local dropItem = items:get(i)
                    ...
                    for _, container in ipairs(destinations) do
                        ...
                        if actualContainer:hasRoomFor(self.character, dropItem:getActualWeight()) then
                            return dropItem, actualContainer
                        end
                    end
                end
            end
...
function JB_GatherItemsAction:getAvailableContainers()
    local containers = {}
...
function JB_GatherItemsAction:new(character, dropSquare, pickupSquares, itemsTable, storageType)
...
function JB_GatherItemsAction:IsDoingSomething()
...
function JB_GatherItemsAction:IsCancel()
...
function JB_GatherItemsAction:GetNextSquare()
...
function JB_GatherItemsAction:GetItemsOnSquare()
...
function JB_GatherItemsAction:PickupItems()
...
function JB_GatherItemsAction:SetDestContainer(item)
...
function JB_GatherItemsAction:SetDestContainerByWeight(type, weight)
...
function JB_GatherItemsAction:DropOffItems()
...
function JB_GatherItemsAction:Start()
...
function JB_GatherItemsAction:End()
...
function JB_GatherItemsAction:Update()
    self.tickCounter = 0
